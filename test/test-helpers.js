  ...
  return db.transaction(trx =>
    ...
    .then(() =>
      Promise.all([
        ...
        trx.raw(`ALTER SEQUENCE registratoion_id_seq minvalue 0 START WITH 1`),
...
function seedUser(db, user) {
  ...
  return db.into('user').insert(preppedUsers)
    .then(() =>
      db.raw(
...
function seedSites(db, testSites) {
  ...
  return db.into('sites').insert(preppedSites)
    .then(() =>
      // update the auto sequence to stay in sync
      db.raw(
        `SELECT setval('sites_id_seq', ?)`,
